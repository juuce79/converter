...
class LengthConversionTests(unittest.TestCase):
    ...
    def test_negative_input(self):
        with self.assertRaises(ValueError) as cm:  # Capture the exception
            self.converter.convert_length('ft', 'm', -10)
        self.print_output('ft', 'm', -10, 'ValueError', str(cm.exception))

    def test_invalid_units(self):
        with self.assertRaises(KeyError) as cm:
            self.converter.convert_length('xyz', 'km', 5)
        self.print_output('xyz', 'km', 5, 'KeyError', str(cm.exception))

    def test_non_numeric_input(self):
        with self.assertRaises(TypeError) as cm:
            self.converter.convert_length('cm', 'm', 'hello')
        self.print_output('cm', 'm', 'hello', 'TypeError', str(cm.exception))
